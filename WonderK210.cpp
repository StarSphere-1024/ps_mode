...
// 构造函数，接受 HardwareSerial 指针和波特率
WonderK210::WonderK210(HardwareSerial *serial, unsigned long baudrate) : _serial(serial)
{
...
// 更新解析函数
void WonderK210::update_data()
{
    // 读取数据
    uint32_t len = _serial->available();
    while (len--)
    {
        int rd = _serial->read();
